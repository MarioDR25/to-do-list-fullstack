...
const FormModal = () => {
    const { isOpen, selectedTask, session, closeModal } = useTaskContext();
    ...
    const handleSubmit = async (e: MySubmitEvent) => {
        e.preventDefault();
        console.log( `soy una tarea nueva: ${formData.title}` );
        
        selectedTask
            ? await actionUpdateTask(selectedTask.id, formData, session.id)
            : await actionAddTask(formData, session.id);
        setFormData({ title: "", description: "", completed: false });
        closeModal();
    };
